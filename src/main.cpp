...
#if defined(NDEBUG)
# error "vmoney cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9a39ce2e4d044bbe08beb6522f26521a05ca29483dc5f421bbe07f88e89af968");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // vmoney: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "vmoney Signed Message:\n";
...
{
    // vmoney: IsDust() detection disabled, allows any valid dust to be relayed.
...
{ nSubsidy = 50000 * COIN; } 
    ...
    nSubsidy >>= (nHeight / 840000); // vmoney: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // vmoney: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // vmoney: 3.0 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // vmoney: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // vmoney: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x45f4dc55740761345a8cb0f0310b915d9c36d00c484ad3816e841df09d4dddce");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC  Newspaper headlines: Russia poisoning broke international law";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("03c602877248b79bf44459c3d108edf7f3eb7f5495f191162f069ba95dff2eb8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526312536;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085274030;

        if (fTestNet)
        {
            block.nTime    = 1526312536;
            block.nNonce   = 385867520;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb9f287bc4f87496146021e6209d6b6b7ba75faedb4bc958f63e61fa66b1837c3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb7, 0xd2 }; // vmoney: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("vmoneyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("vmoneyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static vmoneyMiner(CWallet *pwallet)
{
    printf("vmoneyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vmoney-miner");
    ...
    try { loop {
        ...
        printf("Running vmoneyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&vmoneyMiner, pwallet));
}
